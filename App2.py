...
data = np.load("basin_dataset_gpu_1E6_V2.npz")
rows = data["X"].T
answers = num_to_one_hot(3, data["y"])
...
class GravityApp:
    def __init__(self):
        ...
        self.buttons = {
            ...
            "random": Button(pygame.Rect(bx, 100, 320, 50), "Randomise Points"),
            "predict": Button(pygame.Rect(bx, 160, 320, 50), "Predict Model"),
        }
    ...
    def randomise_points(self):
        index = random.randint(0, rows.shape[1] - 1)
        sample = rows[:, index]

        # [mass_x, mass_y, a1_x, a1_y, a2_x, a2_y, a3_x, a3_y]
    ...
    def draw_panel(self):
        ...
        self.screen.blit(self.font.render(self.prediction_text, True, self.prediction_color), (780, 235))
        self.screen.blit(self.font.render(self.sim_result_text, True, self.sim_result_color), (780, 265))
        ...
        for idx, point in enumerate(self.points):
            if idx < 3:
                ...
                label = f"{point.name}: ({point.x:.2f}, {point.y:.2f}) [drag to move]"
            self.screen.blit(self.small_font.render(label, True, point.color), (780, y))
            y += line_gap
